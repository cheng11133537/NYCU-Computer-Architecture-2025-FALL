...
(
  ...
  // Branch condition logic (X-safe for equality)

  wire   diffSigns_Xhl       = op0_mux_out_Xhl[31] ^ op1_mux_out_Xhl[31];

  // Use 4-state comparisons for BEQ/BNE so unknowns from uninitialized
  // memory/registers do not propagate and wedge the PC selection logic.
  assign branch_cond_eq_Xhl  = ( op0_mux_out_Xhl === op1_mux_out_Xhl );
  assign branch_cond_ne_Xhl  = ( op0_mux_out_Xhl !== op1_mux_out_Xhl );

  // Retain existing arithmetic-based comparisons for signed/unsigned LT/GE
  // which rely on the ALU subtract result sign bit.
  ...
  always @(posedge clk) begin
    if (reset) begin
      for (i_rob = 0; i_rob <= `SLOTS; i_rob = i_rob + 1)
        rob_data[i_rob] <= 32'b0;
    end else if (rob_fill_wen_Whl) begin
  ...
  (
    ...
    // Switch regfile writeback to ROB commit channel
    .wen_p   (rf_wen_Chl),
