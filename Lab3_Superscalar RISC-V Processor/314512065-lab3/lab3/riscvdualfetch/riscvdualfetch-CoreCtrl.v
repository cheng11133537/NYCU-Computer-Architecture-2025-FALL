...
(
  ...
  function [0:0] next_bubble_bit;
    input squash_i;
    input stall_i;
    input bubble_q_i;
    begin
      next_bubble_bit = ( !squash_i && !stall_i ) ? bubble_q_i : 1'b1;
    end
  endfunction
  ...
  function [31:0] sel32;
    ...
    input [31:0] a0;
    input [31:0] a1;
    begin sel32 = (s==1'b0) ? a0 : a1; end
  endfunction

  // Return 1 when a source register should bypass from a given stage
  function [0:0] is_bypass;
    input        rs_en;
    input        rf_wen;
    input  [4:0] rs_addr;
    input  [4:0] rf_waddr;
    input        inst_val;
    begin
      is_bypass = rs_en && rf_wen && (rs_addr == rf_waddr)
                  && !(rf_waddr == 5'd0) && inst_val;
    end
  endfunction

  function [3:0] pick_am_byp;
    input rsAX0;
    input rsAX1;
    input rsAX2;
    input rsAX3;
    ...
    begin
      pick_am_byp = am_r0;
      if      ( rsAX0 ) pick_am_byp = am_AX0_byp;
      else if ( rsAX1 ) pick_am_byp = am_AX1_byp;
      else if ( rsAX2 ) pick_am_byp = am_AX2_byp;
      else if ( rsAX3 ) pick_am_byp = am_AX3_byp;
      else if ( rsAW  ) pick_am_byp = am_AW_byp;
    end
  endfunction

  function [3:0] pick_bm_byp;
    input rsAX0;
    input rsAX1;
    input rsAX2;
    input rsAX3;
    ...
    begin
      pick_bm_byp = bm_r1;
      if      ( rsAX0 ) pick_bm_byp = bm_AX0_byp;
      else if ( rsAX1 ) pick_bm_byp = bm_AX1_byp;
      else if ( rsAX2 ) pick_bm_byp = bm_AX2_byp;
      else if ( rsAX3 ) pick_bm_byp = bm_AX3_byp;
      else if ( rsAW  ) pick_bm_byp = bm_AW_byp;
    end
  endfunction
  ...
  // Next bubble bit
  wire bubble_next_Phl = next_bubble_bit( squash_Phl, stall_Phl, 1'b0 );
  ...
  wire bubble_next_Fhl = next_bubble_bit( squash_Fhl, stall_Fhl, bubble_Fhl );
  ...
  // Instruction memory queue mux 
  wire [31:0] imem0_data_Fhl
    = ( !imemresp0_queue_val_Fhl ) ? imemresp0_msg_data
    : ( imemresp0_queue_val_Fhl )  ? imemresp0_queue_reg_Fhl
    :                               32'bx;

  wire [31:0] imem1_data_Fhl
  ...
  reg [31:0] ir1_Dhl;
  reg        bubble_Dhl;
  reg        dec_second_available;
  reg        decode_idle_Dhl;

  wire dec_stall_lane0;
  wire dec_stall_lane1;

  wire squash_first_D_inst =
    (inst_val_Dhl && !dec_stall_lane0 && dec_stall_lane1);

  // D stage next-state computation (style: compute next-values first)
  wire [31:0] ir0_next_Dhl = stall_Dhl ? ir0_Dhl : imem0_data_Fhl;
  wire [31:0] ir1_next_Dhl = stall_Dhl ? ir1_Dhl : imem1_data_Fhl;
  wire        bubble_reg_next_Dhl = stall_Dhl ? bubble_Dhl : bubble_next_Fhl;

  // On stall, only these two may change based on control events
  wire        dec_second_available_stall_next =
      squash_Dhl                             ? 1'b0 :
      ( dec_issue_fire ? (
          dec_issue_second_pending           ? 1'b0 :
          ( dec_brj_taken_in                 ? 1'b0 : dec_second_available )
        ) : dec_second_available );

  wire        decode_idle_stall_next =
      squash_Dhl                             ? 1'b0 :
      ( dec_issue_fire ? (
          dec_issue_second_pending           ? 1'b1 :
          ( dec_brj_taken_in                 ? 1'b1 :
            ( !dec_second_ready              ? 1'b1 : decode_idle_Dhl ) )
        ) : decode_idle_Dhl );

  wire        dec_second_available_next = (!stall_Dhl) ? (!bubble_next_Fhl)
                                                       : dec_second_available_stall_next;
  wire        decode_idle_next          = (!stall_Dhl) ? 1'b0
                                                       : decode_idle_stall_next;

  always @ ( posedge clk ) begin
    if ( reset ) begin
      bubble_Dhl            <= 1'b1;
      dec_second_available  <= 1'b0;
      decode_idle_Dhl       <= 1'b1;
    end
    else begin
      ir0_Dhl               <= ir0_next_Dhl;
      ir1_Dhl               <= ir1_next_Dhl;
      bubble_Dhl            <= bubble_reg_next_Dhl;
      dec_second_available  <= dec_second_available_next;
      decode_idle_Dhl       <= decode_idle_next;
    end
  end
  ...
  reg  dec_issue_second_pending;
  assign steering_mux_sel_Dhl = dec_issue_second_pending;
  ...
  wire [31:0] dec_inst_sel_bits    = sel32( steering_mux_sel_Dhl, ir0_Dhl, ir1_Dhl );
  wire [31:0] dec_inst_other_bits  = sel32( steering_mux_sel_Dhl, ir1_Dhl, ir0_Dhl );

  wire        dec_inst_sel_valid_raw   = sel1( steering_mux_sel_Dhl, cs0[`RISCV_INST_MSG_INST_VAL], cs1[`RISCV_INST_MSG_INST_VAL] );
  wire        dec_inst_other_valid_raw = sel1( steering_mux_sel_Dhl, cs1[`RISCV_INST_MSG_INST_VAL], cs0[`RISCV_INST_MSG_INST_VAL] );

  wire        dec_inst_sel_valid  = dec_inst_sel_valid_raw;
  wire        dec_inst_other_valid = sel1( steering_mux_sel_Dhl,
                                           dec_second_available,
                                           dec_inst_other_valid_raw );

  wire [1:0] dec_pc_mux_sel_in  = sel2( steering_mux_sel_Dhl, pc_mux_sel_0_Dhl, pc_mux_sel_1_Dhl );
  wire [2:0] dec_br_sel_in      = sel3( steering_mux_sel_Dhl, br_sel_0_Dhl,     br_sel_1_Dhl );
  wire       dec_brj_taken_in   = sel1( steering_mux_sel_Dhl, brj_taken_0_Dhl,  brj_taken_1_Dhl );

  wire [3:0] dec_opA0_byp_sel_in    = sel4( steering_mux_sel_Dhl, op00_byp_mux_sel_Dhl, op10_byp_mux_sel_Dhl );
  wire [1:0] dec_opA0_mux_sel_in    = sel2( steering_mux_sel_Dhl, op00_mux_sel_Dhl,      op10_mux_sel_Dhl );
  wire [3:0] dec_opA1_byp_sel_in    = sel4( steering_mux_sel_Dhl, op01_byp_mux_sel_Dhl,  op11_byp_mux_sel_Dhl );
  wire [2:0] dec_opA1_mux_sel_in    = sel3( steering_mux_sel_Dhl, op01_mux_sel_Dhl,      op11_mux_sel_Dhl );

  wire [3:0] dec_aluA_fn_in          = sel4( steering_mux_sel_Dhl, alu0_fn_Dhl,               alu1_fn_Dhl );
  wire [2:0] dec_md_fn_in            = sel3( steering_mux_sel_Dhl, muldivreq_msg_fn_0_Dhl,    muldivreq_msg_fn_1_Dhl );
  wire       dec_md_val_in           = sel1( steering_mux_sel_Dhl, muldivreq_val_0_Dhl,       muldivreq_val_1_Dhl );
  wire       dec_md_mux_sel_in       = sel1( steering_mux_sel_Dhl, muldiv_mux_sel_0_Dhl,      muldiv_mux_sel_1_Dhl );
  wire       dec_exec_mux_sel_in     = sel1( steering_mux_sel_Dhl, execute_mux_sel_0_Dhl,     execute_mux_sel_1_Dhl );

  wire       dec_is_load_in            = sel1( steering_mux_sel_Dhl, is_load_0_Dhl,          is_load_1_Dhl );
  wire       dec_dmem_rw_in            = sel1( steering_mux_sel_Dhl, dmemreq_msg_rw_0_Dhl,   dmemreq_msg_rw_1_Dhl );
  wire [1:0] dec_dmem_len_in           = sel2( steering_mux_sel_Dhl, dmemreq_msg_len_0_Dhl,  dmemreq_msg_len_1_Dhl );
  wire       dec_dmem_val_in           = sel1( steering_mux_sel_Dhl, dmemreq_val_0_Dhl,      dmemreq_val_1_Dhl );
  wire [2:0] dec_dmemresp_mux_sel_in   = sel3( steering_mux_sel_Dhl, dmemresp_mux_sel_0_Dhl, dmemresp_mux_sel_1_Dhl );
  wire       dec_memex_mux_sel_in      = sel1( steering_mux_sel_Dhl, memex_mux_sel_0_Dhl,    memex_mux_sel_1_Dhl );

  wire       dec_rfA_wen_in    = sel1( steering_mux_sel_Dhl, rf0_wen_Dhl,   rf1_wen_Dhl );
  wire [4:0] dec_rfA_waddr_in  = sel5( steering_mux_sel_Dhl, rf0_waddr_Dhl, rf1_waddr_Dhl );

  wire       dec_csr_wen_in     = sel1( steering_mux_sel_Dhl, csr_wen_0_Dhl,  csr_wen_1_Dhl );
  wire [11:0] dec_csr_addr_in   = sel12( steering_mux_sel_Dhl, csr_addr_0_Dhl, csr_addr_1_Dhl );

  // Steering Logic (style: compute next-values, then drive regs)

  wire       dec_sel_fire = ( inst_val_Dhl && dec_inst_sel_valid );

  wire [31:0] instA_next_Dhl          = dec_sel_fire ? dec_inst_sel_bits         : 32'b0;
  wire [31:0] instB_selected_next_Dhl = dec_inst_other_valid ? dec_inst_other_bits : 32'b0;

  wire [3:0]  opA0_byp_mux_sel_next_Dhl = dec_sel_fire ? dec_opA0_byp_sel_in : am_r0;
  wire [1:0]  opA0_mux_sel_next_Dhl     = dec_sel_fire ? dec_opA0_mux_sel_in : am_rdat;
  wire [3:0]  opA1_byp_mux_sel_next_Dhl = dec_sel_fire ? dec_opA1_byp_sel_in : bm_r1;
  wire [2:0]  opA1_mux_sel_next_Dhl     = dec_sel_fire ? dec_opA1_mux_sel_in : bm_rdat;
  wire [3:0]  aluA_fn_next_Dhl          = dec_sel_fire ? dec_aluA_fn_in       : 4'd0;
  wire [2:0]  muldivreq_msg_fn_next_Dhl = dec_sel_fire ? dec_md_fn_in         : 3'd0;
  wire        muldivreq_val_next_Dhl    = dec_sel_fire ? dec_md_val_in        : 1'b0;
  wire        muldiv_mux_sel_next_Dhl   = dec_sel_fire ? dec_md_mux_sel_in    : 1'b0;
  wire        execute_mux_sel_next_Dhl  = dec_sel_fire ? dec_exec_mux_sel_in  : 1'b0;
  wire        is_load_next_Dhl          = dec_sel_fire ? dec_is_load_in       : 1'b0;
  wire        dmemreq_msg_rw_next_Dhl   = dec_sel_fire ? dec_dmem_rw_in       : 1'b0;
  wire [1:0]  dmemreq_msg_len_next_Dhl  = dec_sel_fire ? dec_dmem_len_in      : 2'd0;
  wire        dmemreq_val_next_Dhl      = dec_sel_fire ? dec_dmem_val_in      : 1'b0;
  wire [2:0]  dmemresp_mux_sel_next_Dhl = dec_sel_fire ? dec_dmemresp_mux_sel_in : 3'd0;
  wire        memex_mux_sel_next_Dhl    = dec_sel_fire ? dec_memex_mux_sel_in : 1'b0;
  wire        rfA_wen_next_Dhl          = dec_sel_fire ? dec_rfA_wen_in       : 1'b0;
  wire [4:0]  rfA_waddr_next_Dhl        = dec_sel_fire ? dec_rfA_waddr_in     : 5'd0;
  wire        csr_wen_next_Dhl          = dec_sel_fire ? dec_csr_wen_in       : 1'b0;
  wire [11:0] csr_addr_next_Dhl         = dec_sel_fire ? dec_csr_addr_in      : 12'd0;
  wire [1:0]  pc_mux_sel_next_Dhl       = dec_sel_fire ? dec_pc_mux_sel_in    : pm_p;
  wire [2:0]  br_sel_next_Dhl           = dec_sel_fire ? dec_br_sel_in        : br_none;
  wire        brj_taken_next_Dhl        = dec_sel_fire ? dec_brj_taken_in     : 1'b0;

  always @(*) begin
    instA_Dhl              = instA_next_Dhl;
    instB_selected_Dhl     = instB_selected_next_Dhl;
    opA0_byp_mux_sel_Dhl   = opA0_byp_mux_sel_next_Dhl;
    opA0_mux_sel_Dhl       = opA0_mux_sel_next_Dhl;
    opA1_byp_mux_sel_Dhl   = opA1_byp_mux_sel_next_Dhl;
    opA1_mux_sel_Dhl       = opA1_mux_sel_next_Dhl;
    aluA_fn_Dhl            = aluA_fn_next_Dhl;
    muldivreq_msg_fn_Dhl   = muldivreq_msg_fn_next_Dhl;
    muldivreq_val_Dhl      = muldivreq_val_next_Dhl;
    muldiv_mux_sel_Dhl     = muldiv_mux_sel_next_Dhl;
    execute_mux_sel_Dhl    = execute_mux_sel_next_Dhl;
    is_load_Dhl            = is_load_next_Dhl;
    dmemreq_msg_rw_Dhl     = dmemreq_msg_rw_next_Dhl;
    dmemreq_msg_len_Dhl    = dmemreq_msg_len_next_Dhl;
    dmemreq_val_Dhl        = dmemreq_val_next_Dhl;
    dmemresp_mux_sel_Dhl   = dmemresp_mux_sel_next_Dhl;
    memex_mux_sel_Dhl      = memex_mux_sel_next_Dhl;
    rfA_wen_Dhl            = rfA_wen_next_Dhl;
    rfA_waddr_Dhl          = rfA_waddr_next_Dhl;
    csr_wen_Dhl            = csr_wen_next_Dhl;
    csr_addr_Dhl           = csr_addr_next_Dhl;
    pc_mux_sel_Dhl         = pc_mux_sel_next_Dhl;
    br_sel_Dhl             = br_sel_next_Dhl;
    brj_taken_Dhl          = brj_taken_next_Dhl;
  end
  ...
  wire       rs10_AX0_byp_Dhl = is_bypass( rs10_en_Dhl, rfA_wen_X0hl, rs10_addr_Dhl, rfA_waddr_X0hl, inst_val_X0hl );
  wire       rs10_AX1_byp_Dhl = is_bypass( rs10_en_Dhl, rfA_wen_X1hl, rs10_addr_Dhl, rfA_waddr_X1hl, inst_val_X1hl );
  wire       rs10_AX2_byp_Dhl = is_bypass( rs10_en_Dhl, rfA_wen_X2hl, rs10_addr_Dhl, rfA_waddr_X2hl, inst_val_X2hl );
  wire       rs10_AX3_byp_Dhl = is_bypass( rs10_en_Dhl, rfA_wen_X3hl, rs10_addr_Dhl, rfA_waddr_X3hl, inst_val_X3hl );
  wire       rs10_AW_byp_Dhl  = is_bypass( rs10_en_Dhl, rfA_wen_Whl,  rs10_addr_Dhl, rfA_waddr_Whl,  inst_val_Whl  );

  wire       rs20_AX0_byp_Dhl = is_bypass( rs20_en_Dhl, rfA_wen_X0hl, rs20_addr_Dhl, rfA_waddr_X0hl, inst_val_X0hl );
  wire       rs20_AX1_byp_Dhl = is_bypass( rs20_en_Dhl, rfA_wen_X1hl, rs20_addr_Dhl, rfA_waddr_X1hl, inst_val_X1hl );
  wire       rs20_AX2_byp_Dhl = is_bypass( rs20_en_Dhl, rfA_wen_X2hl, rs20_addr_Dhl, rfA_waddr_X2hl, inst_val_X2hl );
  wire       rs20_AX3_byp_Dhl = is_bypass( rs20_en_Dhl, rfA_wen_X3hl, rs20_addr_Dhl, rfA_waddr_X3hl, inst_val_X3hl );
  wire       rs20_AW_byp_Dhl  = is_bypass( rs20_en_Dhl, rfA_wen_Whl,  rs20_addr_Dhl, rfA_waddr_Whl,  inst_val_Whl  );

  wire       rs11_AX0_byp_Dhl = is_bypass( rs11_en_Dhl, rfA_wen_X0hl, rs11_addr_Dhl, rfA_waddr_X0hl, inst_val_X0hl );
  wire       rs11_AX1_byp_Dhl = is_bypass( rs11_en_Dhl, rfA_wen_X1hl, rs11_addr_Dhl, rfA_waddr_X1hl, inst_val_X1hl );
  wire       rs11_AX2_byp_Dhl = is_bypass( rs11_en_Dhl, rfA_wen_X2hl, rs11_addr_Dhl, rfA_waddr_X2hl, inst_val_X2hl );
  wire       rs11_AX3_byp_Dhl = is_bypass( rs11_en_Dhl, rfA_wen_X3hl, rs11_addr_Dhl, rfA_waddr_X3hl, inst_val_X3hl );
  wire       rs11_AW_byp_Dhl  = is_bypass( rs11_en_Dhl, rfA_wen_Whl,  rs11_addr_Dhl, rfA_waddr_Whl,  inst_val_Whl  );

  wire       rs21_AX0_byp_Dhl = is_bypass( rs21_en_Dhl, rfA_wen_X0hl, rs21_addr_Dhl, rfA_waddr_X0hl, inst_val_X0hl );
  wire       rs21_AX1_byp_Dhl = is_bypass( rs21_en_Dhl, rfA_wen_X1hl, rs21_addr_Dhl, rfA_waddr_X1hl, inst_val_X1hl );
  wire       rs21_AX2_byp_Dhl = is_bypass( rs21_en_Dhl, rfA_wen_X2hl, rs21_addr_Dhl, rfA_waddr_X2hl, inst_val_X2hl );
  wire       rs21_AX3_byp_Dhl = is_bypass( rs21_en_Dhl, rfA_wen_X3hl, rs21_addr_Dhl, rfA_waddr_X3hl, inst_val_X3hl );
  wire       rs21_AW_byp_Dhl  = is_bypass( rs21_en_Dhl, rfA_wen_Whl,  rs21_addr_Dhl, rfA_waddr_Whl,  inst_val_Whl  );

  // Operand Bypass Mux Select (always @*)

  reg [3:0] op00_byp_mux_sel_Dhl;
  reg [3:0] op01_byp_mux_sel_Dhl;
  reg [3:0] op10_byp_mux_sel_Dhl;
  reg [3:0] op11_byp_mux_sel_Dhl;

  always @* begin
    // Lane 0, operand 0 (A source, am_*)
    op00_byp_mux_sel_Dhl = pick_am_byp(
      rs10_AX0_byp_Dhl, rs10_AX1_byp_Dhl, rs10_AX2_byp_Dhl, rs10_AX3_byp_Dhl, rs10_AW_byp_Dhl
    );

    // Lane 0, operand 1 (B source, bm_*)
    op01_byp_mux_sel_Dhl = pick_bm_byp(
      rs20_AX0_byp_Dhl, rs20_AX1_byp_Dhl, rs20_AX2_byp_Dhl, rs20_AX3_byp_Dhl, rs20_AW_byp_Dhl
    );

    // Lane 1, operand 0 (A source, am_*)
    op10_byp_mux_sel_Dhl = pick_am_byp(
      rs11_AX0_byp_Dhl, rs11_AX1_byp_Dhl, rs11_AX2_byp_Dhl, rs11_AX3_byp_Dhl, rs11_AW_byp_Dhl
    );

    // Lane 1, operand 1 (B source, bm_*)
    op11_byp_mux_sel_Dhl = pick_bm_byp(
      rs21_AX0_byp_Dhl, rs21_AX1_byp_Dhl, rs21_AX2_byp_Dhl, rs21_AX3_byp_Dhl, rs21_AW_byp_Dhl
    );
  end
  ...
  assign dec_stall_lane0 = stall_0_muldiv_use_Dhl || stall_0_load_use_Dhl;
  assign dec_stall_lane1 = stall_1_muldiv_use_Dhl || stall_1_load_use_Dhl;

  // Aggregate Stall Signal

  wire dec_stall_hazard_sel = sel1( steering_mux_sel_Dhl,
                                     dec_stall_lane0,
                                     dec_stall_lane1 );
  wire dec_stall_hazard = dec_stall_hazard_sel || stall_X0hl;

  wire dec_second_ready = sel1( steering_mux_sel_Dhl,
                                dec_second_available,
                                1'b0 );

  wire dec_request_second_issue
    = ( !dec_issue_second_pending )
    && dec_issue_fire
    && dec_second_ready
    && !dec_brj_taken_in;

  wire dec_stall_hold   = dec_issue_second_pending || dec_request_second_issue;


  assign stall_Dhl = dec_stall_hold || dec_stall_hazard;

  // Next bubble bit

  // Compute Decode-stage next bubble bit using helper
  wire bubble_next_Dhl = next_bubble_bit( squash_Dhl, (dec_stall_hazard || decode_idle_Dhl), bubble_Dhl );

  wire dec_issue_fire
    = inst_val_Dhl && dec_inst_sel_valid && !dec_stall_hazard;
  ...
  dec_issue_second_pending <= 1'b0;
      ...
      else if ( dec_issue_fire ) begin
        if ( dec_issue_second_pending == 1'b0 )
          dec_issue_second_pending <= dec_request_second_issue;
        else
          dec_issue_second_pending <= 1'b0;
      end
    end
  ...
  wire bubble_next_X0hl = next_bubble_bit( squash_X0hl, stall_X0hl, bubble_X0hl );
  ...
  wire bubble_next_X1hl = next_bubble_bit( squash_X1hl, stall_X1hl, bubble_X1hl );
  ...
  wire bubble_next_X2hl = next_bubble_bit( squash_X2hl, stall_X2hl, bubble_X2hl );
  ...
  wire bubble_next_X3hl = next_bubble_bit( squash_X3hl, stall_X3hl, bubble_X3hl );
  ...
  localparam [11:0] CSR_ADDR_STATS  = 12'd10;
  localparam [11:0] CSR_ADDR_STATUS = 12'd21;


  `ifndef SYNTHESIS
    wire [31:0] csr_write_data_Whl = (csr_addr_Whl == CSR_ADDR_STATUS) ? 32'd1 : proc2csr_data_Whl;
  `else
  ...
  always @ ( posedge clk ) begin
    if ( csr_wen_Whl && inst_val_Whl ) begin
      case ( csr_addr_Whl )
        CSR_ADDR_STATS  : csr_stats  <= csr_write_data_Whl[0];
        CSR_ADDR_STATUS : csr_status <= csr_write_data_Whl;
      endcase
    end
  end
  ...
  always @ ( posedge clk ) begin
    if (( !cs0[`RISCV_INST_MSG_INST_VAL] && !reset ) 
     || ( !cs1[`RISCV_INST_MSG_INST_VAL] && !reset )) begin
      ...
      if ( illegal_instr_sticky == 1'b1 ) begin
        $finish;
      end

      illegal_instr_sticky = 1'b1;
    end
    else begin
      illegal_instr_sticky = 1'b0;
    end
  end
